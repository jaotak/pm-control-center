...
export async function updateRequirementStatus(reqId: string, newStatus: string, projectId: string) {
    ...
    if (user.role === "DEV" && existing.assigneeId !== user.id) throw new Error("Forbidden");
...
export async function updateUatStatus(uatId: string, newStatus: string, projectId: string) {
    ...
    const existing = await prisma.uATCase.findUnique({ where: { id: uatId }, select: { projectId: true, assigneeId: true } });
...
export async function updateIssueStatus(issueId: string, newStatus: string, projectId: string) {
    ...
    const existing = await prisma.issue.findUnique({ where: { id: issueId }, select: { projectId: true, assigneeId: true } });
