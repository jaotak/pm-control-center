...
export default function CalendarView({
    ...
    // High performance O(N) indexing of tasks by YYYY-MM-DD
            ...
            <div className="bg-white border border-gray-200 rounded-2xl shadow-sm flex flex-col h-[calc(100vh-140px)] min-h-[600px]">
                <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100 shrink-0">
                    ...
                    <div className="flex items-center gap-4">
                        <button
                            onClick={goToday}
                            className="text-sm font-medium text-green-600 hover:bg-green-50 px-3 py-1.5 rounded-lg transition-colors"
                        >
                            วันนี้
                        </button>
                        ...
                        <div className="min-w-[800px] flex flex-col min-h-full">
                            <div className="grid grid-cols-7 border-b border-gray-200 bg-white sticky top-0 z-10 shadow-sm shrink-0">
                                {WEEK_DAYS.map((day, index) => (
                                    <div
                                        key={day}
                                        className={`text-center py-3 text-xs font-semibold ${
                            ...
                            <div className="grid grid-cols-7 gap-px auto-rows-[minmax(120px,_1fr)] flex-1">
                                {calendarDays.map((day, index) => {
                                    if (!day) {
                                        return <div key={`empty-${index}`} className="bg-gray-50/50 min-h-[120px]" />;
                                    }

                                    const dayString = formatDateKey(day);
                                    const isToday = todayStr === dayString;
                                    // O(1) instant lookup from memoized Map
                                    const dayTasks = tasksByDate.get(dayString) || [];

                                    return (
                                        <div
                                            key={dayString}
                                            className="bg-white p-1.5 hover:bg-gray-50 transition-colors flex flex-col group relative min-h-[120px]"
                                        >
                                            <div className="flex justify-between items-start mb-1 shrink-0">
                                                ...
                                                <button
                                                    ...
                                                    className="p-1 text-gray-400 hover:text-green-600 hover:bg-green-50 rounded opacity-0 group-hover:opacity-100 transition-all"
                                                >
                                                    <Plus size={16} />
                                                </button>
                                            </div>
                                                ...
                                                {dayTasks.map((task) => (
                                                    <div key={task.id} className="relative group/task">
                                                        <Link
                                                            ...
                                                            className={`block px-2 py-1.5 text-xs rounded border pr-6 ${
                                                                task.isCompleted
                                                        ...
                                                        >
                                                            <div className="truncate font-medium">
                                                                <span className="font-bold mr-1">
                                                                    {task.project ? task.project.code : "🎯 ทั่วไป"}
                                                                </span>
                                                                {task.title}
                                                            </div>
                                                            {task.assignee && (
...
// Extracted Subcomponent: Isolates modal typing state so calendar grid does not re-render
